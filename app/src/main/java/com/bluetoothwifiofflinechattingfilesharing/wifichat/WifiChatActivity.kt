...
import com.bluetoothwifiofflinechattingfilesharing.adapter.ChatAdapter
...
import com.bluetoothwifiofflinechattingfilesharing.databinding.ActivityWifiP2PBinding
import com.bluetoothwifiofflinechattingfilesharing.databinding.BottomSheetBinding
...
import com.bluetoothwifiofflinechattingfilesharing.utils.PermissionManager
    ...
    */
    ...
    var connectionInfoListener = ConnectionInfoListener { wifiP2pInfo ->
        ...
        if (wifiP2pInfo.groupFormed && wifiP2pInfo.isGroupOwner) {
            ...
            serverClass1 = ServerClass(groupOwnerAddress)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        thisDeviceAddress=localIpAddress

        //Log.d(TAG, "details----${getSystemDetail()} ")
        ...
        chatAdapter = ChatAdapter(this, chatList, Build.MODEL)
    ...
    //this is to get file from storage selection
    val getImageVideos =
        registerForActivityResult(ActivityResultContracts.PickVisualMedia()) {
            // Helper method, refer to FilePickerHelper getPath() method
            // Helper method, refer to FilePickerHelper getPath() method
            fileURI = FilePickerHelper.getPath(this,it!!).toString()
    ...
    private fun setRecycler( receiver: String, msg: String?) {
        val time = getDate(System.currentTimeMillis(), "hh:mm aa")
        val chatModel = chat(0, "", otherDeviceAddress, receiver, R.drawable.happyicon, msg!!, time!!, "", true)
        chatList.add(chatModel)
        chatAdapter.notifyItemInserted(chatList.size - 1)
        binding.chatRecyclerView.scrollToPosition(chatList.size - 1)

        if (isConnected) {
            insertSqlite(otherDeviceAddress, Build.MODEL, msg, time)
        }
    ...
    inner class ClientClass(private val hostAddress1: InetAddress) : Thread() {
        private var hostAddress: String = hostAddress1.hostAddress!!

        override fun run() {
            Log.d(TAG, "client class run ")
            try {
                socket.connect(InetSocketAddress(hostAddress, 8887))
                //set this socket to handle in service

                //Log.d(TAG, "run client: other $hostAddress this $thisDeviceAddress")
                //val name=hostAddress1.hostName
    ...
    inner class ServerClass(val groupOwnerAddress: InetAddress) : Thread() {
        ...
        override fun run() {
            ...
            try {
                serverSocket = ServerSocket(8887)
                //first connect this socket to  server socket the get and out the stream
                socket = serverSocket!!.accept()
                
                //otherDeviceAddress= groupOwnerAddress.hostAddress!!
                //val name=groupOwnerAddress.hostName
                //Log.d(TAG, "run: other $otherDeviceAddress this $thisDeviceAddress")
    ...
    private val mServiceReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (stop.equals("stop")) {
                ...
                isConnected = false
                val time = getDate(System.currentTimeMillis(), "hh:mm aa")
                setStatusSubTitle("last seen at $time")
        ...
        when (msg.what) {
            ...
            STATE_MESSAGE_RECEIVED -> {
                ...
                if (perfix == "serviceStoppedByOther") {
                    ...
                    setRecycler( otherDeviceAddress, receivedMessage)
                }
            ...
            STATE_MESSAGE_SEND -> {
                ...
                setRecycler( Build.MODEL, sentMessage)
    ...
    private fun showSheet() {
        ...
        sheetBinding.tMyName.text = Build.BRAND+" "+Build.MODEL
        
        /*if (thisDeviceName != "") {
            val name = thisDeviceName.toCharArray()
            val drawable =
                ...
                mManager!!.discoverPeers(mChannel, object : WifiP2pManager.ActionListener {
                    ...
                    override fun onFailure(reason: Int) {
                        val failureReason = when (reason) {
                            ...
                            WifiP2pManager.BUSY -> "The system is too busy to perform the requested operation."
                            WifiP2pManager.ERROR -> "An internal error occurred."
                            else -> "An unknown error occurred."
                        }
                        Toast.makeText(this@WifiChatActivity, "Peer discovery failed: $failureReason", Toast.LENGTH_LONG).show()
                        val message = Message.obtain()
                        message.what = STATE_CONNECTION_FAILED
                        handler.sendMessage(message)
                    }
    ...
    fun getDbChats(address: String) {
        CoroutineScope(Dispatchers.IO).launch {
            val list = database!!.insertDao().getOneChat("wifi", address)
            chatList.addAll(list)
            //chatList = ArrayList(list)
            runOnUiThread {  }
            Log.d(ChatActivity.TAG, "getDbChats: chat list fetched${chatList.toString()}")
        }
    }
